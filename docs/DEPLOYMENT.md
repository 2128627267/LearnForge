...
> **编码注意**：`start-dev-lan.bat` 采用 **GBK（ANSI）编码**——中文 Windows 的 cmd.exe 默认按代码页 936 解析批处理，若把此文件另存为 UTF-8，中文字节会被误读并吞掉行尾符，导致解析错乱、整屏"不是内部或外部命令"乱码报错。编辑此文件时请保持 GBK 编码 + CRLF 行尾（`chcp 65001` 方案在 Win10 上亦存在行偏移解析 bug，勿用）。
