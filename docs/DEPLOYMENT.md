...
1. **仅绑定回环地址**：脚本与文档中的启动命令统一使用 `-H 127.0.0.1`，禁止局域网访问（防止外部主机伪造 `Host: localhost` 获取本地访问令牌）。
2. **设置 `LOCAL_ACCESS_TOKEN`**：暴露到网络（含反向代理场景）时必须设置；设置后所有 `/api/*` 请求需携带 `x-local-token` 头，前端会自动从 `/api/local-token`（仅 localhost 可访问）获取。
...
- 旧版 `start-dev.bat` / `stop-dev.bat` 仍可用（仅开发模式，无环境配置功能），推荐迁移到 `setup.ps1`
- 脚本内部显式调用 `npm.cmd`，规避 PowerShell 执行策略对 `npm.ps1` 的限制
- 路径含空格或中文时无需特殊处理（脚本自动切换到项目根目录）

---

*文档更新：2026-08-26（F4 一键部署配置脚本）*
